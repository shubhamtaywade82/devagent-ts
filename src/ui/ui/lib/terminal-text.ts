...
export const removeGraphemeBefore = (value: string, index: number): { value: string; cursor: number } => {
  const graphemes = splitGraphemes(value);
  const safeIndex = Math.max(0, Math.min(index, graphemes.length));
...
export const removeGraphemeAt = (value: string, index: number): { value: string; cursor: number } => {
  const graphemes = splitGraphemes(value);
  const safeIndex = Math.max(0, Math.min(index, graphemes.length));
...
export const truncateToTerminalWidth = (value: string, maximumWidth: number, ellipsis = "…"): string => {
  if (maximumWidth <= 0) {
    return "";
  }
...
export const padToTerminalWidth = (value: string, width: number, alignment: "left" | "right" = "left"): string => {

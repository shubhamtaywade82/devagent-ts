/* eslint-disable no-redeclare -- TS function overloads */
...
export const FocusScope = ({
  ...
  React.useEffect(() => {
    ...
    if (process.env.NODE_ENV !== "production" && activeScopes.size > 0 && !activeScopes.has(scopeId)) {
      console.warn("termcn Ink: multiple non-nested focus scopes are active; only nested overlays are supported.");
    }
  ...
  }, [active, disableFocus, enableFocus, focusById, parentIsActive, returnFocusId, scopeId, stdout]);
...
export function useInteraction(
  ...
  const options =
    typeof optionsOrHandler === "function" ? { ...legacyOptions, onInput: optionsOrHandler } : optionsOrHandler;
  ...
  const isFocused = scope ? scope.active && scope.isTopmost && scope.focusedId === id : nativeFocus.isFocused;
